...
class FakeRawDatagramSocket implements RawDatagramSocket {
  FakeRawDatagramSocket({this.sendError});
  ...
  /// When non-null, [send] throws this error instead of recording the
  /// datagram. Used to simulate iOS EHOSTUNREACH when the multicast
  /// entitlement is missing or Local Network permission is denied.
...
void main() {
  group('DlnaDiscoveryProvider', () {
    ...
    test(
      'surfaces double-send failure as stream error (iOS EHOSTUNREACH)',
      () async {
        final socketException = const SocketException(
          'Send failed (OS Error: No route to host, errno = 65)',
        ...
        final provider = DlnaDiscoveryProvider(
          socketFactory: (host, port) async {
            fakeSocket = FakeRawDatagramSocket(sendError: socketException);
            return fakeSocket;
          },
          httpFetcher: (url) async => _deviceXml,
        );
        ...
        final receivedData = <List<CastDevice>>[];
        final completer = Completer<void>();
        late StreamSubscription<List<CastDevice>> sub;
        sub = stream.listen(
          receivedData.add,
        ...
        // Both sends should have been attempted and failed.
        // (sendError throws before sentData.add runs, so the list stays empty.)
        expect(fakeSocket.sentData, isEmpty);

        // The discovery stream must surface the failure — this is what lets
        // MeowCastService set `_lastDiscoveryAllFailed = true` and unlock
        // the iOS Local Network hint in cast_panel.
        expect(receivedData, isEmpty);
        expect(receivedErrors, hasLength(1));
        expect(receivedErrors.first, isA<StateError>());
        expect(
          (receivedErrors.first as StateError).message,
          contains('DLNA M-SEARCH send failed twice'),
        );
      },
    );

    test(
      'does not surface error when send succeeds (no responses is fine)',
      () async {
        late FakeRawDatagramSocket fakeSocket;
        ...
        // send() returned normally; we cannot distinguish "no devices on LAN"
        // from "kernel silently dropped the packet" — must stay quiet.
        expect(receivedErrors, isEmpty);
        // At least one send should have been recorded.
        expect(fakeSocket.sentData, isNotEmpty);
      },
    );
  });
