...
void main() {
  ...
  group('setupRtspSession flow', () {
    ...
    test(
      'SETUP body advertises timingProtocol=NTP and a non-zero timingPort',
      () async {
        final key = Uint8List(32);
        for (int i = 0; i < 32; i++) {
          key[i] = i;
        ...
        expect(setupRequest, isNotNull);
        // timingProtocol must be NTP (binary plist encodes the string);
        // the ASCII bytes of "NTP" appear in the raw body.
        expect(setupRequest!, contains('NTP'));
    ...
    test(
      'SETUP non-200 throws HapSessionException and skips /feedback + RECORD',
      () async {
        final key = Uint8List(32);
        for (int i = 0; i < 32; i++) {
          key[i] = i;
        ...
        server.listen((sock) async {
          ...
          try {
            // Only answer the first (SETUP) request, with 500.
            final data = await srvSession.readDecryptedData();
            final requestStr = utf8.decode(data, allowMalformed: true);
            receivedMethods.add(requestStr.split('\r\n').first.split(' ').first);
            final resp =
            ...
            sock.add(encrypted);
            await sock.flush();
            // Do NOT answer any further requests; read until the client closes.
            while (true) {
              try {
                final more = await srvSession.readDecryptedData(
                  timeout: const Duration(milliseconds: 300),
                );
                receivedMethods.add(
                  utf8
                      .decode(more, allowMalformed: true)
                      .split('\r\n')
                      .first
                      .split(' ')
                      .first,
        ...
        await expectLater(
          clientSession.setupRtspSession(),
          throwsA(isA<HapSessionException>()),
        );

        // Only SETUP should have been received - no /feedback, no RECORD.
        expect(receivedMethods, equals(['SETUP']));
    ...
    test(
      'SETUP carries Transport/Session/Require headers (RTSP 2326 — '
      'tvOS 17+ 455 Unsupported Transport fix)',
      () async {
        final key = Uint8List(32);
        for (int i = 0; i < 32; i++) {
          key[i] = i;
        ...
        final clientSession = HapSession(
          ...
          host: '127.0.0.1',
          port: server.port,
          sessionId: 'test-transport-session',
        );
        ...
        expect(setupRequest, isNotNull);
        // RFC 2326 §13.1 / RTSP spec — Transport is REQUIRED on SETUP.
        // Apple tvOS 17+ returns 455 Unsupported Transport if absent.
        expect(setupRequest!, contains('Transport: '),
            reason: 'SETUP must advertise an RTP transport profile');
        // The interleaved/record params are what real Apple TV SETUP bodies use
        // (we don't actually transmit RTP frames — buffered playback — but the
        // device still requires the header to accept the request).
        expect(setupRequest!, contains('RTP/AVP/TCP'),
            reason: 'Transport must specify an RTP/AVP profile');
        expect(setupRequest!, contains('interleaved='),
            reason: 'Transport must include interleaved channels');
        expect(setupRequest!, contains('mode=record'),
            reason: 'Transport must request record mode (we are sender)');
        // Session header — pyatv convention; tvOS uses it to bind RECORD /
        // subsequent RTSP requests to the SETUP response Session token.
        expect(setupRequest!, contains('Session: '),
            reason: 'SETUP must include Session header');
        // Require header advertises the AirPlay 2 capability set the client
        // supports. Apple TV uses this to negotiate feature flags; missing
        // it triggers a fallback to V1-only behavior on tvOS 18+.
        expect(setupRequest!, contains('Require: '),
            reason: 'SETUP must declare required AirPlay features');
